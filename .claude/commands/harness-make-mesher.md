...
1. **`harness/attempts_catalog.md` 를 읽어라.** R1~R190+ 시도 이력과 **AVOID 목록**이 있다.
...
2. **fTetWild 원본이 repo 안에 vendored 돼 있다** — `third_party/fTetWild/src/`.
...
3회 실패 시 `harness/_blocked.md` 기록 후 다음 사이클.
...
PASS 시 `harness/attempts_catalog.md` 에 한 줄 추가:
