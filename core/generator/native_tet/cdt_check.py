...
def _edge_tuple(row: np.ndarray) -> Edge:
    return int(row[0]), int(row[1])


def _face_tuple(row: np.ndarray) -> Face:
...
def _tet_triangles(tets: np.ndarray) -> set[tuple[int, int, int]]:
    ...
    return {_face_tuple(row) for row in unique_faces}
...
def _tet_edges(tets: np.ndarray) -> set[tuple[int, int]]:
    ...
    edges = np.stack(
        [tets[:, pair_idx[:, 0]], tets[:, pair_idx[:, 1]]],
        axis=2,
    ).reshape(-1, 2)
    edges.sort(axis=1)
    unique_edges = np.unique(edges, axis=0)
    return {_edge_tuple(row) for row in unique_edges}
...
def check_edge_recovery_chained(
    ...
    *,
    ...
    if F_surf.size == 0:
        ...
        src_se: np.ndarray = F_surf[:, [0, 1, 2]].reshape(-1).astype(np.int64)
        dst_se: np.ndarray = F_surf[:, [1, 2, 0]].reshape(-1).astype(np.int64)
...
def _check_edge_recovery_python(
    F: np.ndarray,
    tets: np.ndarray,
) -> CDTCheckResult:
    """NumPy oracle/fallback for strict edge and face recovery."""
    # surface edge set.  Tuple rows avoid packed-key overflow for sparse/high
    # provenance indices.  This is slower than the native C++23 transaction,
    # but remains an exact independent oracle when the extension is absent.
    surface_edge_rows = np.column_stack(
        (
            np.minimum(F[:, [0, 1, 2]], F[:, [1, 2, 0]]).reshape(-1),
            np.maximum(F[:, [0, 1, 2]], F[:, [1, 2, 0]]).reshape(-1),
        )
    )
    unique_surface_edges = np.unique(surface_edge_rows, axis=0)
    surf_edges = {_edge_tuple(row) for row in unique_surface_edges}

    tet_edges = _tet_edges(tets)

    missing = sorted(surf_edges.difference(tet_edges))

    tet_faces = _tet_triangles(tets)
    unique_surface_faces = np.unique(np.sort(F, axis=1), axis=0)
    surf_faces = {_face_tuple(row) for row in unique_surface_faces}
    n_face_present = sum(1 for face in surf_faces if face in tet_faces)
...
def _validated_index_array(values: np.ndarray, *, columns: int, name: str) -> np.ndarray:
    ...
    if raw.size and np.issubdtype(raw.dtype, np.unsignedinteger):
        if int(raw.max()) > np.iinfo(np.int64).max:
            raise ValueError(f"{name} contains an index outside int64 range")
    result: np.ndarray = np.asarray(raw, dtype=np.int64)
...
def check_edge_recovery(
    ...
    """입력 surface F 의 edge 와 face 가 tet mesh 에 존재하는지 검사.
    ...
    Returns:
        CDTCheckResult.
    """
    F = _validated_index_array(F, columns=3, name="F")
    tets = _validated_index_array(tets, columns=4, name="tets")
    if F.size == 0:
    ...
    if (
        native is not None
        and hasattr(native, "audit_cdt_constraints")
        and F.flags.c_contiguous
        and tets.flags.c_contiguous
    ):
        values = native.audit_cdt_constraints(F, tets)
        missing_array = np.asarray(values["missing_edges"], dtype=np.int64)
        return CDTCheckResult(
            n_surface_edges=int(values["n_surface_edges"]),
            n_present_as_tet_edges=int(values["n_present_as_tet_edges"]),
            n_missing=int(values["n_missing"]),
            missing_edges=[_edge_tuple(row) for row in missing_array],
            n_surface_faces=int(values["n_surface_faces"]),
            n_present_as_tet_faces=int(values["n_present_as_tet_faces"]),
            n_missing_faces=int(values["n_missing_faces"]),
        )
    return _check_edge_recovery_python(F, tets)
