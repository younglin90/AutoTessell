...
def _bl_layer_quality_proxy(
    ...
    *,
    ...
    """Return BL-local (max_non_ortho, min_face_weight, max_skewness).

    This is a lightweight accept/reject proxy for in-generator smoothing.  It
    deliberately inspects only internal faces touching inserted BL cells, so it
    can guard a candidate layer move without running the full evaluator.
    """
    n_internal = min(len(neighbour), len(faces))
    if n_internal <= 0 or not owner:
        return 0.0, 1.0, 0.0
    ...
    for fi in range(n_internal):
        ...
        nbr = int(nbr_arr[fi])
        if own < base_n_cells and nbr < base_n_cells:
            continue
        if own < 0 or nbr < 0 or own >= len(centres) or nbr >= len(centres):
            continue
        ...
        if d2 <= 1e-30:
            continue
        d_mag = float(np.sqrt(d2))
        face = faces[fi]
        normal, area = _face_normal_area(points, face)
        if area > 1e-30:
            cos_theta = abs(float(np.dot(normal, d)) / d_mag)
            cos_theta = min(1.0, max(0.0, cos_theta))
            max_non_ortho = max(
                max_non_ortho,
                float(np.degrees(np.arccos(cos_theta))),
            )

        fc = _face_centroid(points, face)
        diff = fc - centres[own]
        t = float(np.dot(diff, d) / d2)
        min_face_weight = min(min_face_weight, t, 1.0 - t)
        proj = centres[own] + t * d
        max_skewness = max(
            max_skewness,
            float(np.linalg.norm(fc - proj) / d_mag),
        )

    return float(max_non_ortho), float(min_face_weight), float(max_skewness)


def _smooth_inner_layers_tangential_guarded(
    fp: np.ndarray,
    wall_vert_indices: list[int],
    wall_tri_verts: dict[int, tuple[int, int, int]],
    wall_face_indices: list[int],
    ...
    *,
    ...
    relax: float = 0.35,
    n_iter: int = 1,
) -> tuple[np.ndarray, int, bool, tuple[float, float, float], tuple[float, float, float]]:
    """SMESH-style guarded tangential smoothing for internal BL layer nodes.

    SMESH viscous layers smooth inflated LayerEdge target nodes and reject moves
    that worsen local simplex quality.  This clean-room analogue only moves
    intermediate layer points tangentially in their local extrusion plane, keeps
    the wall and deepest/core vertices fixed, then accepts the whole batch only
    if BL-local non-orthogonality/face-weight/skewness does not regress and at
    least one of them improves.
    """
    empty_quality = (0.0, 1.0, 0.0)
    if num_layers < 3 or not wall_vert_indices or len(layer_point_ids) < num_layers + 1:
        return fp, 0, False, empty_quality, empty_quality
    ...
    valid_v = [
        ...
        if v in base_lp
        and v in deepest_lp
        and all(v in layer_point_ids[k] for k in range(num_layers + 1))
    ]
    if len(valid_v) < 3:
        return fp, 0, False, empty_quality, empty_quality

    valid_set = set(valid_v)
    adj: dict[int, set[int]] = {v: set() for v in valid_v}
    for fi in wall_face_indices:
        verts = wall_tri_verts.get(fi)
        if not verts:
            continue
        for i in range(3):
            a = int(verts[i])
    ...
    before = _bl_layer_quality_proxy(
        fp, faces, owner, neighbour, base_n_cells=base_n_cells,
    )
    candidate = fp.copy()
    n_moved = 0

    for _ in range(max(1, int(n_iter))):
        next_points = candidate.copy()
        for layer_i in range(1, num_layers):
            lid = layer_point_ids[layer_i]
            for v in valid_v:
                nbs = [nb for nb in adj.get(v, ()) if nb in lid]
                if len(nbs) < 2:
                    continue

                idx = lid[v]
                base = candidate[base_lp[v]]
                deepest = candidate[deepest_lp[v]]
                axis = deepest - base
                ...
                cur = candidate[idx]
                nb_pts = np.asarray([candidate[lid[nb]] for nb in nbs], dtype=np.float64)
                centroid = nb_pts.mean(axis=0)
                delta = centroid - cur
                tangential = delta - float(np.dot(delta, axis)) * axis
                move_len = float(np.linalg.norm(tangential))
                if move_len <= 1e-14:
                    continue

                nb_dist = np.linalg.norm(nb_pts - cur, axis=1)
                max_move = 0.20 * float(np.median(nb_dist)) if nb_dist.size else 0.0
                if max_move <= 1e-14:
                    continue
                if move_len > max_move:
                    tangential *= max_move / move_len

                next_points[idx] = cur + float(relax) * tangential
                n_moved += 1
        candidate = next_points

    if n_moved == 0:
        return fp, 0, False, before, before

    after = _bl_layer_quality_proxy(
        candidate, faces, owner, neighbour, base_n_cells=base_n_cells,
    )
    before_non, before_weight, before_skew = before
    after_non, after_weight, after_skew = after
    no_regress = (
        after_non <= before_non + 1e-9
        and after_weight >= before_weight - 1e-9
        and after_skew <= before_skew + 1e-9
    )
    improved = (
        after_non < before_non - 0.05
        or after_weight > before_weight + 1e-4
        or after_skew < before_skew - 1e-4
    )
    if no_regress and improved:
        return candidate, n_moved, True, before, after
    return fp, 0, False, before, after
...
def generate_native_bl(
    ...
    *,
    ...
    # SMESH viscous-layer analogue: smooth intermediate inflated nodes in the
    # tangent plane, but accept only if a BL-local quality proxy improves.
    if (
        _os.environ.get("AUTO_TESSELL_BL_SMESH_TANGENTIAL_SMOOTH", "1") != "0"
        and len(wall_vert_indices) >= 10
        and layer_point_ids
        and cfg.num_layers >= 3
        ...
        try:
            (
                ...
                _smesh_before,
                _smesh_after,
            ) = _smooth_inner_layers_tangential_guarded(
                final_points,
                wall_vert_indices,
                wall_tri_verts,
                wall_face_indices,
                ...
                relax=float(
                    _os.environ.get("AUTO_TESSELL_BL_SMESH_TANGENTIAL_RELAX", "0.35")
                ),
                n_iter=int(
                    _os.environ.get("AUTO_TESSELL_BL_SMESH_TANGENTIAL_ITERS", "1")
                ),
            )
            log.info(
                "native_bl_smesh_tangential_smooth",
                component="native_bl",
                phase="smesh_viscous_layer",
                accepted=bool(_smesh_accepted),
                n_moved=int(_n_smesh_moved),
                before_non_ortho=round(float(_smesh_before[0]), 3),
                after_non_ortho=round(float(_smesh_after[0]), 3),
                before_face_weight=round(float(_smesh_before[1]), 6),
                after_face_weight=round(float(_smesh_after[1]), 6),
                before_skew=round(float(_smesh_before[2]), 6),
                after_skew=round(float(_smesh_after[2]), 6),
            )
        except Exception as exc:
            log.warning(
                "native_bl_smesh_tangential_smooth_skipped",
                reason=str(exc)[:120],
            )
