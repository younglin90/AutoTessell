"""Private C++23 conforming cube-lattice Native Tri BL adapter."""
...
def _refusal(reason: str, requested_layers: int = 0) -> dict[str, Any]:
    return {
        "accepted": False,
        "status": "native_tri_authoritative_cube_lattice_bl_refinement_refused",
        "reason": reason,
...
def _certificate(source_certificate: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise ValueError("cube_lattice_source_certificate_payload_invalid")
    return value


def make_authoritative_cube_lattice_template_anchor(
    ...
    *,
    ...
    component: str,
    provenance: str,
    template_id: str = "native-tri-authoritative-cube-lattice-v1",
) -> dict[str, Any]:
    ...
    if len(faces) != 2 or faces[0] >= faces[1]:
        raise ValueError("cube_lattice_source_face_pair_required")
    if len(edges) != 4 or len(active) != 4:
        raise ValueError("cube_lattice_four_edge_registration_required")
    if not digest or not preflight_receipt.get("preflight_accepted", False):
        raise ValueError("cube_lattice_preflight_receipt_required")
    return {
        "schema": "NativeTriAuthoritativeCubeLatticeBL/v1",
...
def write_native_tri_authoritative_cube_lattice_bl(
    source_certificate: Mapping[str, Any],
    ...
    *,
    ...
    if requested_layers < 0:
        return _refusal("cube_lattice_requested_layers_invalid", requested_layers)
    try:
        kernel = import_native_extension(
            "native_tri_authoritative_cube_lattice_bl_refinement"
        )
        return dict(kernel.write_native_tri_authoritative_cube_lattice_bl(
        ...
        return _refusal(
            f"native_tri_authoritative_cube_lattice_unavailable:{type(exc).__name__}",
            requested_layers,
        )
