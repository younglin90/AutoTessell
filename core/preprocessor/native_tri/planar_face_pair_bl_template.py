"""Private C++23 authority-bound planar Native Tri face-pair BL adapter."""
...
def _refusal(reason: str, requested_layers: int = 0) -> dict[str, Any]:
    return {
        "accepted": False,
        "status": "native_tri_planar_face_pair_bl_template_refused",
        "reason": reason,
...
def _certificate(source_certificate: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise ValueError("pair_source_certificate_payload_invalid")
    return value


def make_planar_face_pair_template_anchor(
    ...
    *,
    ...
    component: str,
    provenance: str,
    template_id: str = "native-tri-planar-face-pair-v1",
) -> dict[str, Any]:
    ...
    if len(faces) != 2 or faces[0] >= faces[1]:
        raise ValueError("pair_source_face_ids_required")
    if len(edges) != 4 or len(active) != 4:
        raise ValueError("pair_four_edge_registration_required")
    digest = str(preflight_receipt.get("preflight_digest", ""))
    if not digest or not preflight_receipt.get("preflight_accepted", False):
        raise ValueError("pair_preflight_receipt_required")
    return {
        "schema": "NativeTriPlanarFacePairTemplate/v1",
...
def write_native_tri_planar_face_pair_bl(
    source_certificate: Mapping[str, Any],
    ...
    *,
    ...
    if requested_layers < 0:
        return _refusal("pair_requested_layers_invalid", requested_layers)
    try:
        kernel = import_native_extension("native_tri_planar_face_pair_bl_template")
        return dict(kernel.write_native_tri_planar_face_pair_bl(
        ...
        return _refusal(
            f"native_tri_planar_face_pair_bl_unavailable:{type(exc).__name__}",
            requested_layers,
        )
