"""Private C++23 authority-bound diagonal-front constrained-Delaunay Native Tri BL adapter."""
...
def _refusal(reason: str, requested_layers: int = 0) -> dict[str, Any]:
    return {
        "accepted": False,
        "status": "native_tri_authority_bound_diagonal_front_cdt_refused",
        "reason": reason,
...
def _certificate(source_certificate: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise ValueError("diagonal_front_cdt_source_certificate_payload_invalid")
    return value


def make_authority_bound_diagonal_front_cdt_template_anchor(
    ...
    *,
    ...
    component: str,
    provenance: str,
    template_id: str = "native-tri-authority-bound-diagonal-front-cdt-v1",
) -> dict[str, Any]:
    ...
    if len(faces) != 2 or faces[0] >= faces[1]:
        raise ValueError("diagonal_front_cdt_source_face_pair_required")
    if len(edges) != 4 or len(active) != 4:
        raise ValueError("diagonal_front_cdt_four_edge_registration_required")
    if not digest or not preflight_receipt.get("preflight_accepted", False):
        raise ValueError("diagonal_front_cdt_preflight_receipt_required")
    return {
        "schema": "NativeTriAuthorityBoundDiagonalFrontCDT/v1",
...
def write_native_tri_authority_bound_diagonal_front_cdt_bl(
    source_certificate: Mapping[str, Any],
    ...
    *,
    ...
    if requested_layers < 0:
        return _refusal("diagonal_front_cdt_requested_layers_invalid", requested_layers)
    try:
        kernel = import_native_extension(
            "native_tri_authority_bound_diagonal_front_cdt"
        )
        return dict(kernel.write_native_tri_authority_bound_diagonal_front_cdt_bl(
        ...
        return _refusal(
            f"native_tri_authority_bound_diagonal_front_cdt_unavailable:{type(exc).__name__}",
            requested_layers,
        )
