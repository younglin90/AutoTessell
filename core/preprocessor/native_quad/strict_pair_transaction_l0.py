...
def materialize_strict_quad_pair_transaction_l0(
    ...
    *,
    ...
    if (
        not isinstance(pair_plan, np.ndarray)
        or pair_plan.dtype != np.dtype(np.int64)
        or pair_plan.ndim != 2
        or pair_plan.shape[1] != 2
        or not pair_plan.flags.c_contiguous
        or len(pair_plan) == 0
        or len(source_triangles) != 2 * len(pair_plan)
    ):
        return StrictQuadPairTransactionResult(
            False,
            "reject_strict_quad_pair_plan",
            "strict_quad_pair_plan_invalid",
            False,
            False,
            None,
        )
    ...
    for first, second in pair_plan.tolist():
        if (
            first < 0
            or second < 0
            or first >= len(source_triangles)
            or second >= len(source_triangles)
        ):
            return StrictQuadPairTransactionResult(
                False,
                "reject_strict_quad_pair_plan",
    ...
    quad_patches = [
        source_patches[int(pair[0])] if len(source_patches) > int(pair[0]) else None
        for pair in pair_plan
    ]
    source_groups = (
        source_physical_groups.source_face_groups
        if isinstance(source_physical_groups, AuthoritativePhysicalGroupMapping)
        else ()
    )
    quad_groups = [
        source_groups[int(pair[0])] if len(source_groups) > int(pair[0]) else ""
        for pair in pair_plan
    ]
