...
5-tier hybrid pipeline (tried in order):
  ...
  Tier 0   tessell_mesh / geogram  (BSD-3-Clause)
             ...
             engines/legacy/tessell_mesh/ 빌드 필요 (./build.sh)
...
def generate_mesh(
    ...
    try:
        ...
        prefix = "(tessell 미빌드 — cd engines/legacy/tessell_mesh && ./build.sh)\n" if tessell_skipped else ""
...
def _tessell_pipeline(stl_path: Path, case_dir: Path, bbox: BBox) -> dict:
    """
    C++/geogram tet 메쉬 생성.
    engines/legacy/tessell_mesh/ 빌드 후 products/web/api/mesh/tessell_mesh.so 필요.
    """
    try:
        import tessell_mesh as tm  # type: ignore[import]
    except ImportError:
        raise _TessellNotBuilt(
            "tessell_mesh.so 없음 — 빌드: cd engines/legacy/tessell_mesh && ./build.sh"
        )
