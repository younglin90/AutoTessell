...
- Clean exact head `43a2f720`, Release Unix Makefiles, first-party native
  install ON, and optional adapters OFF passed fresh c1 validation.
- Configure `3.060754147 s`; serial native-build contract target
  `58.330337793 s`; contract verification `0.067885505 s`; postflight was
  clean.  Evidence:
  `/tmp/autotessell-c196-native-gnu-clean-runner-evidence.json`.

### Release state

- Gate 2 remains `UNVERIFIED` pending native-install matrix evidence.  No
...
- The TRI+QUAD current validation runner used concurrency `1` and passed in
  `4.758139906 s`:
  `/tmp/autotessell-c173-tri-quad-current-runner-evidence.json`.
- TRI+QUAD remains an independent default-off, unrouted product.  This does
  not claim or relabel native Tri or strict Quad behavior.
...
- The retained actual native-Tet sphere three-run corpus now binds the
  immutable source, complete `polyMesh` identities, quality-report byte
  hashes, typed native product contract, and report-internal Gate-4
  source/output attestations.  All three retain aggregate `c002...5996b`.
- Corpus evidence:
  `/tmp/autotessell-c165-tet-sphere-corpus-evidence.json`; runner evidence:
  `/tmp/autotessell-c165-tet-sphere-corpus-runner-evidence.json`.  The bounded
  runner used concurrency `1`, took `0.039 s`, and is expected `ERROR` /
  `UNVERIFIED` with exit `1`: release evidence remains incomplete by design.
...
### Native-Poly cube full current-corpus binding

- The retained actual native-Poly cube three-run corpus now binds the
  immutable source, complete `polyMesh` identities, quality-report byte
  hashes, typed native product contract, and report-internal Gate-4
  source/output attestations.  All three retain aggregate `c8d5...e53e7`.
- Corpus evidence:
  `/tmp/autotessell-c163-poly-cube-corpus-evidence.json`; runner evidence:
  `/tmp/autotessell-c163-poly-cube-corpus-runner-evidence.json`.  The bounded
  runner used concurrency `1`, took `0.040 s`, and is expected `ERROR` /
  `UNVERIFIED` with exit `1`: release evidence remains incomplete by design.
...
- Two strict native-Poly cube runs passed at concurrency `1` in `32.56 s` and
  `31.49 s`; native checker reports `0` negative volumes.  Exact `polyMesh`
  hash prefixes: `points fa58...`, `faces c985...`, `owner e8d7...`,
  `neighbour 9fed...`, `boundary 630c...`.
- This is two serial runs of one fixture/configuration only, so Gate 9 remains
  `UNVERIFIED`.  Gate-4 authority remains unverified; no `PASS` claim follows.
  The retained obsolete native-Poly protected branch is unaffected.
...
- Two additional full strict native-Tet sphere CLI runs passed at concurrency
  `1` in `15.85 s` and `15.46 s`; with Cycle 118's first run, all three have
  byte-identical SHA-256 values for `polyMesh` `points`, `faces`, `owner`,
  `neighbour`, and `boundary`.
- This adds a second serial fixture only.  Thread/order matrix and
  cross-product corpus evidence remain absent, so Gate 9 remains `UNVERIFIED`.
...
- Two full strict native-Tet cube CLI runs passed at concurrency `1` in
  `9.94 s` and `9.30 s`.  `polyMesh` `points`, `faces`, `owner`, `neighbour`,
  and `boundary` SHA-256 values are byte-identical between those runs.
- This is a two-run, one-fixture, one-thread/order-configuration observation
  only.  The required third run and configuration/corpus coverage remain
  absent, so Gate 9 remains `UNVERIFIED`.
