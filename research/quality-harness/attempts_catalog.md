...
| R201 | BETA2266_VVV9I2_ENVELOPE_DISTANCE_HELPER | tet | PASS | =0.0 | fTetWild §3.2 envelope distance primitive, _envelope_distance_to_triangles(pts, V_in, F_in) → centroid-based distance (N,) lower-bound approximation, skeleton-only (no caller, default OFF), KDTree pre-filter + centroid distance query, regression 35/35 PASS (70s), worst_mq 0.208±0.005 maintained, BL_OK 5/5, commit 2a9ce31 |
| R202 | BETA2267_VVV9I3_DIAG_HOOK | tet | PASS | =0.0 | fTetWild §3.2 envelope distance diagnostic hook in mesher.py (VVV12 try-block), _envelope_distance_to_triangles call (gate OFF, mesh unchanged), log emit native_tet_vvv9i_diag per-fid (n_invasion, max_dist, eps, wall_ms), evidence-only skeleton, regression 31/31 PASS (66s), worst_mq 0.208±0.005 maintained, BL_OK 5/5, commit 4a57688 |
| R204 | BETA2269_VVV9J1_SLIM_LOCAL_JACOBIAN | tet | PASS | =0.0 | SLIM local Jacobian helper skeleton (Rabinovich 2017 §3), _slim_local_jacobian_per_tet(V, F, tets) → dict(F, det_F, frob_F2), no caller/default OFF, regression 35/35 PASS (67s), worst_mq 0.208±0.005 stable, BL_OK 5/5, bench 3.22s, commit PENDING |
| R205 | BETA2270_VVV9J2_SLIM_SD_ENERGY | tet | PASS | =0.0 | Symmetric Dirichlet energy helper (Rabinovich 2017 §3 eq.3), _slim_symmetric_dirichlet_energy(jac_dict) → E_SD(T,) per-tet + cof(F) adjugate closed-form, no caller/default OFF, regression 31/31 PASS (67s), worst_mq 0.208±0.005 stable, bench 0.72s, commit PENDING |
| R206 | BETA2271_VVV9J3_SLIM_LOCAL_GRADIENT | tet | PASS | =0.0 | Symmetric Dirichlet gradient helper (Rabinovich 2017 §3), _slim_compute_local_gradient(jac_dict) → ∂E/∂F(T,3,3) per-tet trace projection, no caller/default OFF, regression 31/31 PASS (65.4s), worst_mq 0.208±0.005 stable, bench 59.2s, commit PENDING |
| R207 | BETA2272_VVV9J3B_LINE_SEARCH | tet | PASS | =0.0 | Armijo backtracking line-search helper (Rabinovich 2017 §3 Algorithm 1 step 3), _slim_armijo_line_search(pts, tets, v_idx, d_v, max_step) → dict(alpha, E_post, accepted, n_iter_used), skeleton-only (no caller/default OFF), monotone descent guard, regression 35/35 PASS (67.75s), worst_mq 0.208±0.005 stable, bench 0.72s, commit PENDING |
| R208 | BETA2273_VVV9J4_NEWTON_COMPOSE | tet | PASS | =0.0 | Full SLIM Newton step compose helper (Rabinovich 2017 §3 Algorithm 1), _slim_newton_step_one_vertex(pts, tets, v_idx, max_step) → dict(new_pos, energy_delta, n_iter_used, accepted), chains jac→grad→line-search→apply (simulation-only, mesh unchanged), skeleton-only (no caller/default OFF), regression 31/31 PASS (65.38s), worst_mq 0.127 stable, bench 59.1s, commit eb60a62 |
| R209 | BETA2274_VVV9K1_PRIORITY_QUEUE_MAIN | tet | PASS | =0.0 | Priority queue integration helper (Klingner 2008 §3.3 Algorithm 3), _priority_queue_main_loop(pts, tets, qs, max_iters, time_budget_ms) → per-tet quality optimization via _priority_queue_attempt_improvement, monotone guards (R197/R208 hardening chained), skeleton-only (no caller/default OFF), regression 35/35 PASS (67.6s), worst_mq 0.208±0.005 stable, bench 67.59s, BL_OK 5/5, commit PENDING |
| R210-R219 | BETA2275-BETA2284 (VVV9K2-K8) | tet | PASS | series | diag→apply→hardening chain for K-line (priority queue), worst_mq 0.208 ±0.005 maintained across 10 cards, BL_OK 5/5 stable, cumulative bench 67.4s avg, all commits 35/35 PASS regression, latest R220 commit f56fe0f |
| R220 | BETA2285_VVV9K8_HARDEN_MONOTONE | tet | PASS | =0.0 | K-line monotone + early-exit hardening finalization (R197/R208/R220 skeleton chain completion), monotone guard + stagnation early-exit (n_no_improve≥K), worst_mq 0.208±0.005 (stable), BL_OK 5/5, regression 35/35 PASS (66.63s), bench 67.59s, commit f56fe0f |
| R221 | BETA2286_VVV9N1_LINE_COMPARE_HELPER | tet | PASS | =0.0 | Klingner 2008 §6 ablation skeleton: _evidence_compare_lines(pts, tets, sliver_q_thr, max_apply) → dry-simulate H/J/K lines on deepcopy, compare Δworst_mq per-line, worst-line selector (read-only, no caller/default OFF), monotone safety inherited from R197/R208/R220 helpers, regression 35/35 PASS (66.63s), worst_mq 0.208±0.005 stable, BL_OK 5/5, bench ~67.5s est., commit PENDING |
| R224 | BETA2289_VVV9P1_MULTI_FACE_REMOVAL_HELPER | tet | PASS | =0.0 | Klingner 2008 §3.2 multi-face removal candidates skeleton: _multi_face_removal_candidates(pts, tets, q_thr, k_worst) → list[(tet_idx, faces_to_remove, quality_post)], evidence enumeration only (no caller/default OFF), wall safety inherited (sliver-pre>=1, worst<0.10), regression 35/35 PASS (66.63s), worst_mq 0.208±0.005 stable, BL_OK 5/5, bench 67.5s, commit 8a29d6a |
| R225 | BETA2290_VVV9P2_DIAG_HOOK | tet | PASS | =0.0 | diagnostic hook in VVV12 (gate OFF, read-only): _multi_face_removal_candidates helper call + sliver-pre guard, log emit native_tet_vvv9p_diag per-fid (n_candidates, top_quality, wall_ms), mesh unchanged (evidence discard), regression 35/35 PASS (66.63s), worst_mq 0.208±0.005 stable, BL_OK 5/5, bench 67.59s, commit 11888de |
| R226 | BETA2291_VVV9P3_DIAG_ON | tet | PASS | =0.0 | gate flip True (mesher.py:3138 _VVV9P_DIAG=False→True), evidence collection enabled (sliver-pre>=1+worst<0.10 wall safety maintained). log emit native_tet_vvv9p_diag per-fid, mesh unchanged (helper discard), regression 31/31 PASS (66.2s), worst_mq 0.208±0.005 stable, BL_OK 5/5, bench 59.1s, commit 459b051 |
| R-c6 | BETA2827 | tet | **BLOCKED** | n/a | scale-relative 3-2 flip: maker 실측 회귀 (flip 후 하류 BETA2826 smooth 가 owner-centroid 를 경계로 밀어 bskew 재악화 — 블록 단계 guard 로는 미탐지). 부차: planner/maker 가 서로 다른 ad-hoc 스크립트로 baseline 측정 (4159 vs 280, 프로토콜 불일치 — PYTHONHASHSEED 5종 실험으로 비결정성 아님 확정). 재계획 조건: scripts/smoke_native_cylinder.py 로 re-baseline + guard 를 post-smooth(최종) 단계로. maker_block.txt 참조 |
| R-c7 | BETA2829 | tet | **DIAGNOSIS-ONLY** | skew 44.9 불변 | 실린더 잔여 boundary-skew 44.9 의 근본원인 규명 + 3 노선 사망 확정 (전부 실측·revert). 원인: cylinder.stl 측벽 z-ring 이 2개뿐(z=±0.5) → 벽-conforming tet 이 구조적으로 full-height flat cap (벽 3정점 + 4th 정점이 벽 근접, normal_dist=height/4 미소 → bskew 폭발). **AVOID (실측 반증)**: (1) post-smooth flip_edges_32/faces_23 — skew 44.9 불변(quality-gate) 또는 2.1e15 폭발(any-valid, relative-inv guard revert). (2) boundary-edge split — flat cap 을 쪼개면 더 작은 flat cap 만 늘어 44.9→67→78 악화. (3) locked-smooth 재적용 — 280 폭발. **해법은 near-wall 내부점 삽입(Garimella offset ring)뿐** — volume-only local op 으론 불가. 여러 카드짜리. gate-OFF 진단자 코드는 미채택(죽은 코드 방지), 지식만 보존. |
