...
def _on_payment_succeeded(db: Session, job_id: str, payment_intent_id: str) -> None:
    ...
    # Enqueue Celery task — if broker is unavailable, mark failed and refund immediately
    ...
    try:
        ...
        # Use the worker's refund helper (fresh session, issues Stripe refund)
        from worker.tasks import _mark_failed_and_refund
        _mark_failed_and_refund(str(job_id), "Failed to start mesh task — a full refund has been issued")
