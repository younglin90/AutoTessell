"""Test-only whole-SSS bypass comparison for the sphere strict-source failure."""
...
def _worker(repeat: int, mode: str, case_dir: Path) -> dict[str, object]:
    ...
    if mode == "sss_disabled":
        os.environ["AUTO_TESSELL_P3_SSS_REVIVAL"] = "0"
    elif mode == "current":
        os.environ["AUTO_TESSELL_P3_SSS_REVIVAL"] = "1"
    else:
...
def test_sphere_whole_sss_bypass_feasibility(tmp_path: Path) -> None:
    current = tuple(_run(tmp_path, "current", repeat) for repeat in range(3))
    disabled = tuple(_run(tmp_path, "sss_disabled", repeat) for repeat in range(3))

    assert _signature(current[0]) == _signature(current[1]) == _signature(current[2])
    assert _signature(disabled[0]) == _signature(disabled[1]) == _signature(disabled[2])
    assert current[0]["first_source_failure"] == "post_sss_pass0_relocate_pre_accept"
    assert disabled[0]["first_source_failure"] is None
    assert current[0]["result"]["success"] is False
    assert disabled[0]["result"]["success"] is False
    assert current[0]["result"]["writer"] is False
    assert disabled[0]["result"]["writer"] is False
