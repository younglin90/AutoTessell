#!/usr/bin/env python3
"""poly + cfMesh BL 21-STL bench (autoresearch-deep poly loop).

Parallels ``bench_hex_cavity_eval.py`` but routes the 21-STL set
through the poly base method (``--mesh-type poly --tier cfmesh_poly``)
with BL produced natively by cfMesh's ``pMesh`` (polyhedral).

Outputs:

  - tests/stl/bench_poly_cavity_eval_result.json (per-STL records)
  - tests/stl/bench_poly_cavity_eval_summary.tsv (one row per STL)
...
RUN_ROOT = Path(
    os.environ.get(
        "AUTO_TESSELL_BENCH_POLY_RUN_ROOT",
        "/tmp/autotessell_bench_poly_cavity_eval",
    )
)
...
def main() -> int:
    ...
    if not stls:
        print("[bench_poly_cavity_eval] no STLs discovered", file=sys.stderr)
        return 2
    print(f"[bench_poly_cavity_eval] {len(stls)} STLs, RUN_ROOT={RUN_ROOT}")
    ...
    print(f"[bench_poly_cavity_eval] wrote {json_path}")
    print(f"[bench_poly_cavity_eval] wrote {tsv_path}")
