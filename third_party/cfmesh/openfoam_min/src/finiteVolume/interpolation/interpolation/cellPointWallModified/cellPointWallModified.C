...
#include "cellPointWallModified.H"
#include "syncTools.H"
...
Foam::interpolations::cellPointWallModified<Type>::calcPointField
(
...
) const
{
    ...
    // Interpolate to the points with wall patches extrapolated
    {
        ...
        Foam::volPointInterpolation::New(mesh).interpolate
        (
            psiExtrapolated,
            psip
        );
...
template<class Type>
Foam::interpolations::cellPointWallModified<Type>::cellPointWallModified
(
    const VolField<Type>& psi
)
:
    cellPoint<Type>(psi, calcPointField(psi))
{}
...
Foam::interpolations::cellPointWallModified<Type>::cellPointWallModified
(
    const cellPointWallModified<Type>& i
)
:
    cellPoint<Type>(i)
{}
